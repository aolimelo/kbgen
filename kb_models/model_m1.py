from load_tensor_tools import loadGraphNpz, loadTypesNpz, load_types_dict, load_relations_dict, load_type_hierarchy, load_prop_hierarchy, load_domains, load_ranges
...
class KBModelM1(KBModel):
    ...
    def check_for_quadratic_relations(self):
        ...
        for r in self.rel_densities.keys():
            ...
            if func>10 and inv_func>10:
                ...
                if density > 0.1:
                    self.logger.debug(r, func, inv_func, density, n_obj, n_subj, reflex)
    ...
    def synthesize_entities(self,g,n_entities):
        ...
        for i in self.dist_types.keys():
            entitiy_types_dict[i] = []
        entitiy_types = choice(self.dist_types.keys(), n_entities, True, normalize(self.dist_types.values()))
        type_assertions = 0
        pbar = tqdm.tqdm(total=n_entities)
        for i in range(n_entities):
            entitiy_types_dict[entitiy_types[i]].append(i)
            entity_i = URIEntity(i).uri
            for t_i in entitiy_types[i].types:
